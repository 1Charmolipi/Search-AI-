...
# design PriorityQueue using heapq
# references: https://github.com/hikariakio/sokoban-python-ai/blob/main/search.py
import heapq
class PriorityQueue:
...
class Problem:
    ...
    # design heuristic function using manhattanDistance and linearConflict
    def heuristic(self, node):
        totalDistance = 0

        for stonePos, _ in node.state.stones:
            closestGoalDistance = min(manhattanDistance(stonePos, goal) for goal in self.goals)
            conflictPenalty = sum(linearConflict(stonePos, goal, [otherStone for otherStone, _ in node.state.stones]) for goal in self.goals)
            totalDistance += closestGoalDistance + conflictPenalty
            
        return totalDistance
    
# https://www.sciencedirect.com/science/article/pii/S0004370215000867 : 1.1.3. Heuristics for Sokoban
def linearConflict(stone, goal, other_stones):
    # increase the heuristic value by two when a pair of adjacent stones is in the optimal path of each other
    if stone[0] == goal[0]:
        for other_stone in other_stones:
            if other_stone[0] == stone[0] and min(stone[1], goal[1]) < other_stone[1] < max(stone[1], goal[1]):
                return 2
    if stone[1] == goal[1]:
        for other_stone in other_stones:
            if other_stone[1] == stone[1] and min(stone[0], goal[0]) < other_stone[0] < max(stone[0], goal[0]):
                return 2
    return 0
...
# the A* algorithm is similar to the UCS algorithm
# the only thing need to change is to add the heuristic function to lambda
def aStar(problem, f = None):
...
import time
import tracemalloc
def a_star(input,output):
    problem = readInput(input)

    start_time = time.time()
    tracemalloc.start()

    node, nodeGenerated = aStar(problem, f = lambda node: node.pathCost + problem.heuristic(node))
